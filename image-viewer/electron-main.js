  ...
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      if (req.url.startsWith('/api/images')) {
        ...
        req.on('end', () => {
          try {
            logMessage(`Move to basket request body: "${body}"`, 'info');
            ...
            // Build path using forward slashes first
            const fullPathStr = path.join(publicDir.replace(/\\/g, '/'), normalizedPath);
            // Then convert to OS-specific path
            const sourceFullPath = fullPathStr.replace(/\//g, path.sep);
            ...
            const basketDir = path.join(publicDir, 'basket');
            ...
            logMessage(`Moved to basket: ${normalizedPath} -> ${path.relative(publicDir, destPath)}`, 'success');
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ 
              success: true,
              oldPath: normalizedPath,
              newPath: path.relative(publicDir, destPath)
            }));
          } catch (error) {
